...
//Nos serviría manejo de un motor, desconectar, estados de conexión y errores.
public class BTCommunicator extends Thread {
    ...
    public static final int STATE_DISCONNECTING = 1007;
    ...
    public void run() {
        ...
        catch (IOException e) { sendState(STATE_RECEIVEERROR);}
        
      //Hace que reciba los broadcast de bluetooth generales, incluyendo 
        //los broadcasts que hace cuando se ha desconectado. 
        BroadcastReceiver receiver = new BroadcastReceiver(){
        	@Override
        	public void onReceive(Context context, Intent intent){
        		String accion = intent.getAction();
        		if(BluetoothDevice.ACTION_ACL_DISCONNECT_REQUESTED.equals(accion))
        		{
        			//Si alguien se quiere desconectar como corresponde envia el
        			//mensaje apropiado
        			sendState(STATE_DISCONNECTING);
        			
        		}
        		else if(BluetoothDevice.ACTION_ACL_DISCONNECTED.equals(accion))
        		{
        			//Se desconecto a la mala, enviar mensaje de error
        			sendState(STATE_RECEIVEERROR);
        		}
        	}
        };
        
        if(myOwner instanceof MainActivity)
        {
        	//Queremos que el broadcast este registrado desde el padre, si es que puede.
        	IntentFilter filter_disconnected = new IntentFilter(BluetoothDevice.ACTION_ACL_DISCONNECTED);
            IntentFilter filter_disrequest = new IntentFilter(BluetoothDevice.ACTION_ACL_DISCONNECT_REQUESTED);
            ((MainActivity) myOwner).registerReceiver(receiver,filter_disconnected);
            ((MainActivity) myOwner).registerReceiver(receiver,filter_disrequest);
        }
