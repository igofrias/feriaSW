...
public class MainActivity extends SlidingFragmentActivity implements BTConnectable{
  ...
  //llama al supa framento
    void launch_gameselect() {//
		FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
		SherlockFragment fragment = ((GameList_Fragment)getSupportFragmentManager().findFragmentByTag("gameselect"));
		
		if(fragment==null){
			fragment = new GameList_Fragment();
			ft.replace(R.id.linear, fragment,"gameselect");
		}
		else{
			if(fragment.isDetached()){
				ft.attach(fragment);
   ...
   //llama al supa framento
   void detach_gamelist() {//identificamos y cargamos el fragmento menu
		FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
		SherlockFragment fragment = ((GameList_Fragment)getSupportFragmentManager().findFragmentByTag("gameselect"));
