...
/*
	Copyright (C) 2008 - 2012 Georgiy Vasylyev 
*/
...
AppMan = new function(){
	...
	this.clearValidation = function(){
		...
		for (var prop in this.ufoForms.forms) {
			this.ufoForms.forms[prop].length = 0;			 
		}		
		for (var prop in this.ufoForms.submits) {
...
ufo.copyTranslations = function(el){
	var config = AppMan.Utils.getConfig('AppTranslations');
	var hash = AppMan.Utils.idSplit(el.id)[0];
	var cfrom = AppMan.Utils.idJoin(hash, 'CopyFrom');
	var cto = AppMan.Utils.idJoin(hash, 'CopyTo');
	config.a = {};
	config.a.cfrom = jQuery('#'+cfrom).val();
	config.a.cto = jQuery('#'+cto).val();
	config.a = AppMan.JSON.encode(config.a);
	config.m = 'copyResources';
	AppMan.Filter.filter(config);
}
...
var ufoCf = new function(){
	...
	this.moveSettingsForm = function(){
		...
		var newheight = Math.max(newheight, parheight);
	...
	this.addCustomField = function(el, config){
		...
		var config = AppMan.Utils.getConfig(config);
